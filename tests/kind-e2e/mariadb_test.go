...
// mariadb_test.go validates the full MariaDB backup/restore flow against the in-cluster
// mariadb:11 server, using S3 (LocalStack) as the storage backend.
// dumpscript transparently falls back to mariadb-dump when the engine type is mariadb.
...
var _ = Describe("MariaDB backend (mariadb-dump → S3 → mariadb restore)", Ordered, func() {
	const (
		name        = "mariadb-e2e"
		prefix      = "mariadb-e2e"
		job         = "mariadb-e2e-manual"
		restoreName = "mariadb-restore-e2e"
	)
...
spec:
  ...
  database:
    type: mariadb
    host: mariadb.%s.svc.cluster.local
    name: testdb
...
`, name, testNamespace, dumpscriptImg, testNamespace, bucketName, prefix, localstackInCluster)
	...
	BeforeAll(func() {
		By("seeding the marker row in MariaDB (mariadb_marker)")
		mariadbExec("CREATE TABLE IF NOT EXISTS mariadb_marker (id INT AUTO_INCREMENT PRIMARY KEY, val VARCHAR(64));")
		mariadbExec("TRUNCATE TABLE mariadb_marker;")
		mariadbExec("INSERT INTO mariadb_marker (val) VALUES ('kind-mariadb-marker');")

		By("applying MariaDB BackupSchedule")
	...
	It("CronJob env contains DB_TYPE=mariadb and points at the in-cluster mariadb Service", func() {
		...
		Expect(out).To(ContainSubstring("DB_TYPE=mariadb"))
		Expect(out).To(ContainSubstring("DB_HOST=mariadb." + testNamespace))
		Expect(out).To(ContainSubstring("DB_NAME=testdb"))
	})

	It("backup job uploads a mariadb dump to S3", func() {
		...
		Eventually(func() (string, error) {
			...
			if failed == "True" {
				return "Failed", fmt.Errorf("MariaDB backup job failed — captured pod logs:\n%s", capturedLogs)
			}
			return "", nil
	...
	It("Restore from S3 recovers the data into MariaDB", func() {
		...
		mariadbExec("DROP TABLE IF EXISTS mariadb_marker;")
...
`, restoreName, testNamespace, backupKey, dumpscriptImg, testNamespace, bucketName, localstackInCluster)
		...
		out := mariadbExec("SELECT val FROM mariadb_marker WHERE val = 'kind-mariadb-marker';")
		Expect(out).To(ContainSubstring("kind-mariadb-marker"))
	})
})
