...
// redis_test.go validates the Redis backup flow against the in-cluster redis:7
// server, using S3 (LocalStack) as the storage backend.
//
// Restore is intentionally not exercised: dumpscript's redis restorer returns
// ErrRedisRestoreUnsupported. The test covers backup only.
...
var _ = Describe("Redis backend (redis-cli --rdb → S3)", Ordered, func() {
	const (
		name   = "redis-e2e"
		prefix = "redis-e2e"
		job    = "redis-e2e-manual"
	)
...
`, name, testNamespace, dumpscriptImg, testNamespace, bucketName, prefix, localstackInCluster)
	...
	BeforeAll(func() {
		By("seeding a marker key in Redis")
		redisCmd("SET", "kind-redis-marker", "ok")

		By("applying Redis BackupSchedule")
	...
	It("CronJob env contains DB_TYPE=redis and points at the in-cluster redis Service", func() {
		...
		Expect(out).To(ContainSubstring("DB_TYPE=redis"))
		Expect(out).To(ContainSubstring("DB_HOST=redis." + testNamespace))
	})

	It("backup job uploads an RDB snapshot to S3", func() {
		...
		Eventually(func() (string, error) {
			...
			if failed == "True" {
				return "Failed", fmt.Errorf("Redis backup job failed — captured pod logs:\n%s", capturedLogs)
			}
			return "", nil
